...
class ETLProcess:
    ...
    def transform_data(self, file_path):
        ...
        df["rating_average"] = df["rating_average"].astype(
            "float64"
        )  # DOUBLE PRECISION
    ...
    def load_product_dim_table(self):
        ...
        ].to_sql("product_dim_table", self.engine, if_exists="append", index=False)
    ...
    def load_sales_fact_table(self):
        ...
        ].to_sql("sales_fact_table", self.engine, if_exists="append", index=False)
        print("Loaded data successfully")

    def load_date_dim_table(self):
        print("Loading data to destination")
        date_dim_table = self.df[
            ["date", "day", "month", "year", "quarter"]
        ].drop_duplicates()
        date_dim_table.to_sql(
            "date_dim_table", self.engine, if_exists="append", index=False
        )
        print("Loaded data successfully")
